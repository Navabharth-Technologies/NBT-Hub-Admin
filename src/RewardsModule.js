                ...
                <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: winWidth < 600 ? 'column' : 'row', justifyContent: 'space-between', alignItems: winWidth < 600 ? 'flex-start' : 'flex-start', marginBottom: '32px', gap: '20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                            <div onClick={onBack || (() => {})} style={{ cursor: onBack ? 'pointer' : 'default', background: '#f8fafc', padding: '10px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ 
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', borderRight: winWidth < 768 ? 'none' : '1.5px solid rgba(255,255,255,0.1)', borderBottom: winWidth < 768 ? '1.5px solid rgba(255,255,255,0.1)' : 'none', paddingBottom: winWidth < 768 ? '20px' : '0' }}>
                            ...
                            <div>
                                <p style={{ margin: '0 0 5px 0', fontSize: winWidth < 480 ? '8px' : '9px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1.5px' }}>Member Status</p>
                                <h3 style={{ margin: 0, fontSize: winWidth < 480 ? '16px' : (winWidth < 768 ? '18px' : '24px'), fontWeight: '950', color: '#ffffff' }}>Active Hub</h3>
                            </div>
                        </div>
                        ...
                        <div style={{ textAlign: winWidth < 768 ? 'left' : 'center', borderRight: winWidth < 768 ? 'none' : '1.5px solid rgba(255,255,255,0.1)', borderBottom: winWidth < 768 ? '1.5px solid rgba(255,255,255,0.1)' : 'none', paddingBottom: winWidth < 768 ? '20px' : '0' }}>
                            <p style={{ margin: '0 0 5px 0', fontSize: winWidth < 480 ? '8px' : '9px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1.5px' }}>Total Points Circulating</p>
                            <h3 style={{ margin: 0, fontSize: winWidth < 480 ? '18px' : (winWidth < 768 ? '22px' : '28px'), fontWeight: '950', color: '#facc15' }}>
                                {rewards.reduce((acc, r) => acc + (Number(r.points) || 0), 0).toLocaleString()} <span style={{ fontSize: winWidth < 480 ? '12px' : '18px' }}>REP</span>
                            </h3>
                        </div>

                        {/* Leaderboard Score */}
                        <div style={{ textAlign: winWidth < 768 ? 'left' : 'right' }}>
                            <p style={{ margin: '0 0 5px 0', fontSize: winWidth < 480 ? '8px' : '9px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1.5px' }}>Top Recognition</p>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: winWidth < 768 ? 'flex-start' : 'flex-end', gap: '10px' }}>
                                <h3 style={{ margin: 0, fontSize: winWidth < 480 ? '14px' : (winWidth < 768 ? '18px' : '22px'), fontWeight: '950', color: '#ffffff' }}>
                                    {leaderboard[0] ? leaderboard[0].name : "Syncing..."}
                                </h3>
                                <Star size={winWidth < 480 ? 16 : 20} color="#facc15" fill="#facc15" />
                            </div>
                        </div>
                    </div>
                                    ...
                                    {!selectedHistoryUser ? (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                                <div>
                                                    <h2 style={{ margin: 0, fontSize: winWidth < 480 ? '16px' : '20px', fontWeight: '1000', color: '#0f172a' }}>Recognition Glimpse</h2>
                                                    <p style={{ margin: '5px 0 0 0', fontSize: winWidth < 480 ? '10px' : '12px', color: '#94a3b8', fontWeight: '700' }}>Aggregated results per member</p>
                                                </div>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: winWidth < 480 ? '8px' : '15px' }}>
                                                    {Array.from(new Set(filteredRewards.map(r => r.employee_id))).length > 5 && (
                                                        <button 
                                                            onClick={() => setShowAllFeed(!showAllFeed)}
                                                            style={{ 
                                                                background: 'none', border: 'none', color: '#3863a8', fontSize: winWidth < 480 ? '9px' : '11px', fontWeight: '900', cursor: 'pointer', textTransform: 'uppercase', textDecoration: 'underline' 
                                                            }}>
                                                            {showAllFeed ? 'View Less' : 'View All'}
                                                        </button>
                                                    )}
                                                    <div style={{ fontSize: winWidth < 480 ? '9px' : '12px', fontWeight: '800', color: '#3b82f6', background: '#eff6ff', padding: winWidth < 480 ? '4px 8px' : '6px 12px', borderRadius: '10px' }}>
                                                    ...
                                                    return (
                                                        <div key={empId} onClick={() => setSelectedHistoryUser(empId)} style={{ background: 'white', padding: winWidth < 768 ? '16px' : '20px', borderRadius: winWidth < 768 ? '20px' : '24px', border: '1.5px solid #f1f5f9', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'transform 0.2s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: winWidth < 768 ? '12px' : '15px' }}>
                                                                <div style={{ width: winWidth < 480 ? '34px' : (winWidth < 768 ? '40px' : '45px'), height: winWidth < 480 ? '34px' : (winWidth < 768 ? '40px' : '45px'), borderRadius: '12px', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #f1f5f9' }}><Award size={winWidth < 480 ? 18 : (winWidth < 768 ? 20 : 22)} color="#0369a1" /></div>
                                                                <div>
                                                                    <div style={{ fontSize: winWidth < 480 ? '12px' : (winWidth < 768 ? '14px' : '15px'), fontWeight: '1000', color: '#0f172a' }}>{resolveEmployeeName(empId)}</div>
                                                                    <div style={{ fontSize: winWidth < 480 ? '9px' : (winWidth < 768 ? '10px' : '11px'), color: '#64748b', fontWeight: '700' }}>
